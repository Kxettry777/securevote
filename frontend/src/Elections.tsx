...
import type { RegisteredParty, RegistryData, Election, ElectionDetail, Voter } from "./api";
...
type Props = { createMode?: boolean; onCreate?: () => void; onList?: () => void; initialElectionId?: string; onCreated?: (id: string) => void; token: string; admin: boolean; voter: boolean; onSessionEnd: (message: string) => void };
...
function statusAt(election: Election, now: number) {
  return electionPhase(election.startsAt, election.endsAt, now);
}

function ScheduleForm({ election, parties = [], busy, save }: { election?: Election; parties?: RegisteredParty[]; busy: boolean; save: (body: object) => Promise<boolean> }) {
...
function ElectionView({ id, token, admin, voter, onSessionEnd, back, onChange, now, syncClock }: Props & { id: string; back: () => void; onChange: () => void; now: number; syncClock: (value: string) => void }) {
  const [data, setData] = useState<ElectionDetail | null>(null);
  const [available, setAvailable] = useState<Voter[]>([]);
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api<ElectionDetail>(`/elections/${id}`, {}, token), admin ? api<{ voters: Voter[] }>("/admin/voters", {}, token) : Promise.resolve({ voters: [] })])
      .then(([detail, users]) => { if (active) { syncClock(detail.election.serverTime); setData(detail); setAvailable(users.voters); setError(""); } })
      .catch(err => {
        if (!active) return;
        setData(null);
  ...
  async function mutate(path: string, method: string, body: object | undefined, success: string) {
    ...
    try {
      ...
      const detail = await api<ElectionDetail>(`/elections/${id}`, {}, token);
      syncClock(detail.election.serverTime); setData(detail); setMessage(success); onChange();
      return true;
    } catch (err) {
      if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message);
...
export function Elections(props: Props) {
  const { token, admin, voter, onSessionEnd } = props;
  const [showRemoved, setShowRemoved] = useState(false);
  const [removing, setRemoving] = useState<Election | null>(null);
  const [message, setMessage] = useState("");
  ...
  const [registeredParties, setRegisteredParties] = useState<RegisteredParty[]>([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [revision, setRevision] = useState(0);
  ...
  const syncClock = useCallback((value: string) => {
    const time = Date.parse(value);
    if (Number.isFinite(time)) { clock.current = { time, observedAt: performance.now() }; setNow(time); }
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api<{ elections: Election[] }>(showRemoved ? "/elections?removed=true" : "/elections", {}, token), admin ? api<RegistryData>("/registry", {}, token) : Promise.resolve(null)]).then(([result, registry]) => { if (active && registry) setRegisteredParties(registry.parties); if (active) { if (result.elections[0]) syncClock(result.elections[0].serverTime); setElections(result.elections); setError(""); } })
      .catch(err => {
        if (!active) return;
        if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message);
        else setError(err.message);
  ...
  async function changeRemoval(election: Election, restore = false) {
    setBusy(true); setError(""); setMessage("");
    try {
      await api(`/elections/${election.id}${restore ? "/restore" : ""}`, { method: restore ? "POST" : "DELETE" }, token);
      setRemoving(null); setElections(current => current.filter(item => item.id !== election.id));
      setMessage(restore ? "Election restored." : "Election removed. You can restore it from Removed elections.");
    } catch (err) {
      if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message);
      else setError(err instanceof Error ? err.message : "The election could not be updated.");
    } finally { setBusy(false); }
  }
  if (selected) return <ElectionView key={selected} {...props} id={selected} now={now} syncClock={syncClock} back={() => { setSelected(""); setRevision(value => value + 1); }} onChange={() => setRevision(value => value + 1)} />;
  ...
  return <div className="election-workspace">
    ...
    {admin && !creating && <div className="button-row" role="group" aria-label="Election visibility"><button className={showRemoved ? "secondary-button" : "primary-button"} aria-pressed={!showRemoved} disabled={busy || loading} onClick={() => { if (showRemoved) { setLoading(true); setShowRemoved(false); } }}>Current elections</button><button className={showRemoved ? "primary-button" : "secondary-button"} aria-pressed={showRemoved} disabled={busy || loading} onClick={() => { if (!showRemoved) { setLoading(true); setShowRemoved(true); } }}>Removed elections</button></div>}
    <section className="welcome-panel"><div><span className="panel-kicker">{admin ? "Election planning" : voter ? "Your elections" : "Election overview"}</span><h2>{admin ? "Prepare your next party election." : voter ? "Know the parties. Make your choice." : "Follow elections and verify results."}</h2><p>{admin ? "Start with registered parties and their submitted candidate rosters, then set the schedule and assign eligible voters." : voter ? "Explore your assigned elections, review the parties, and vote when voting opens." : "Review election schedules and party manifestos, then inspect confirmed results after voting ends."}</p></div></section>
    <section className="panel"><div className="section-heading"><h2>{creating ? "Prepare an election" : voter ? "Assigned elections" : "Elections"}</h2><div className="button-row"><button className="secondary-button" disabled={loading || busy} onClick={() => { setLoading(true); setRevision(value => value + 1); }}>Refresh</button>{admin && <button className="primary-button" disabled={busy} onClick={() => { if (props.onCreate && !creating) props.onCreate(); else if (creating && props.onList) props.onList(); else setCreating(!creating); setError(""); }}>{creating ? "Cancel" : "New election"}</button>}</div></div>
      {error && <p className="notice error" role="alert">{error}</p>}
      {creating && !loading && <ScheduleForm parties={registeredParties} busy={busy} save={async body => {
        setBusy(true); setError("");
        try {
          const result = await api<ElectionDetail>("/elections", { method: "POST", body: JSON.stringify(body) }, token);
          setCreating(false); setSelected(result.election.id); setRevision(value => value + 1); props.onCreated?.(result.election.id); return true;
        } catch (err) {
          if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message);
          else setError(err instanceof Error ? err.message : "Election could not be created.");
          return false;
        } finally { setBusy(false); }
      }} />}
