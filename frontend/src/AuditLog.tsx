...
type Entry = { id: string; action: string; targetId: string; electionTitle: string | null; actorName: string; createdAt: string };
type Page = { entries: Entry[]; nextCursor: string | null; latestId: string | null };
const actions: Record<string, string> = { election_removed: "Election removed", election_restored: "Election restored", voter_registered_by_admin: "Voter registered by admin", party_registered: "Party and account registered", party_registration_updated: "Party registration updated", candidate_role_created: "Candidate role created", candidate_role_updated: "Candidate role updated", candidate_registered: "Candidate nominated", registered_candidate_updated: "Nomination updated", registered_candidate_removed: "Nomination removed", party_roster_submitted: "Party roster submitted", election_banner_updated: "Election banner updated", election_banner_removed: "Election banner removed", election_created: "Election created", election_updated: "Election updated", candidate_added: "Party registered", candidate_updated: "Party updated", candidate_removed: "Party removed", voter_assigned: "Voter assigned", voter_unassigned: "Voter assignment removed", voter_approved: "Voter approved", voter_revoked: "Voter approval revoked" };

export function AuditLog({ token, onSessionEnd }: { token: string; onSessionEnd: (message: string) => void }) {
  ...
  const [removing, setRemoving] = useState<{ entry?: Entry; throughId?: string } | null>(null);
  const [revision, setRevision] = useState(0);
  useEffect(() => {
    let mounted = true;
    api<Page>(`/admin/audit${cursor ? `?before=${cursor}` : ""}`, {}, token).then(page => { if (mounted) { setData(page); setError(""); } })
      .catch(err => { if (!mounted) return; setData(null); if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message); else setError(err.message); })
  ...
  async function remove() {
    ...
    try {
      const result = await api<{ deleted: number }>(`/admin/audit${removing.entry ? `/${removing.entry.id}` : ""}`, { method: "DELETE", body: JSON.stringify({ confirmation: "DELETE", throughId: removing.throughId }) }, token);
      setRemoving(null); setMessage(`${result.deleted} audit ${result.deleted === 1 ? "entry" : "entries"} deleted.`); refresh();
    } catch (err) {
      if (err instanceof ApiError && (err.status === 401 || err.status === 403)) onSessionEnd(err.message);
      else setError(err instanceof Error ? err.message : "Audit deletion failed.");
    } finally { setBusy(false); }
  }
  return <div className="election-workspace"><section className="welcome-panel"><div><span className="panel-kicker">Administrative history</span><h2>Follow the decisions behind each election.</h2><p>Review changes to elections, political parties, voter approval, and eligibility.</p></div></section>
