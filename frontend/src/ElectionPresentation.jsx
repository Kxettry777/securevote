...
export function ElectionCountdown({ election, now, compact = false }) {
    const phase = electionPhase(election.startsAt, election.endsAt, now);
    if (phase === "ended")
        return <div className="election-countdown ended"><span className="countdown-dot" aria-hidden="true"/><strong>Voting ended</strong></div>;
...
export function PartyMark({ party }) {
...
export function PartyRoster({ party }) {
...
export function ElectionBanner({ election, token, compact = false }) {
    const [source, setSource] = useState(null);
    const [failedVersion, setFailedVersion] = useState(null);
    useEffect(() => {
        if (!election.bannerVersion)
            return;
        ...
        fetch(`/api/elections/${election.id}/banner`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal })
            ...
            setFailedVersion(election.bannerVersion); });
...
export function BannerEditor({ election, busy, save }) {
    ...
    return <form className="election-form banner-editor" onSubmit={async (event) => { event.preventDefault(); if (file && await save(file))
        clear(); }}>
    ...
    <fieldset disabled={busy}><label>Choose banner image<input key={inputKey} type="file" accept="image/png,image/jpeg,image/webp" onChange={event => {
            ...
            if (next.size > 4 * 1024 * 1024) {
                setError("The banner must be 4 MB or smaller.");
                return;
            }
            setFile(next);
