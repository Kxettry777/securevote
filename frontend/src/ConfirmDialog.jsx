...
export function ConfirmDialog({ title, children, busy, permanent = false, action, onConfirm, onCancel }) {
    ...
    return <dialog ref={ref} className="confirm-dialog" aria-labelledby="confirmation-title" onCancel={event => { event.preventDefault(); if (!busy)
        onCancel(); }}>
    <h2 id="confirmation-title">{title}</h2>{children}
    <form className="election-form" onSubmit={event => { event.preventDefault(); if (!busy && (!permanent || confirmation === "DELETE"))
        onConfirm(); }}>
